...
3. **Dnešní program**: Domácí úkoly, funkce.
...
- Čím méně úrovní vnoření, tím lepší (= čitelnější, srozumitelnější kód). Používejte `return` a `sys.exit()` pro snížení úrovně vnoření.
...
del seznam		# smaže všechny položky seznamu a smaže také proměnnou "seznam".
```
