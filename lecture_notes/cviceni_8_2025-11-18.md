...
**append / extend vs. prealokace**

Nejjednodušší způsob, jak změřit časovou náročnost nějaké kusu kódu je použít "magické slovíčko" `%%timeit` v IPythonu:
...
In [1]: %%timeit
   ...
   ...:
   ...:
37.1 ms ± 961 μs per loop (mean ± std. dev. of 7 runs, 10 loops each)
...
In [2]: %%timeit
   ...
   ...:
   ...:
24.7 ms ± 503 μs per loop (mean ± std. dev. of 7 runs, 10 loops each)

```

Jak vlastně funguje `list.append` a jaká je jeho složitost? Model: 

- Prázdné pole se nealokuje prázdné, ale s nějakou zvolenou velikosti pro k položek.
