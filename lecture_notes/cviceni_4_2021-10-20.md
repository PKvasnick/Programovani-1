...
3. `if !__name__=="__main__":`označuje blok kódu, který se vykoná pouze je-li daný soubor spoštěn přímo - tedy ne importován jiným souborem. V druhém případě dostane soubor jméno `__name__` souboru. Pro toto cvičení, kde nebudeme vyvíjet Pythonovské moduly,tento kód **nepotřebujete**. 
